...
namespace MessagePipeSampleCodes
{
    ...
    [RequestHandlerFilter(typeof(ReqResHandlerFilterLevel2))]
    ...
    public class ReqResHandlerFilterLevel1 : RequestHandlerFilter<Request, Response>
    {
    ...
    public class ReqResHandlerFilterLevel2 : RequestHandlerFilter<Request, Response>
    {
        public override Response Invoke(Request request, Func<Request, Response> next)
        {
            // 강화된 잘못된 패킷 처리
            if (request._type < 1 || request._type > 10)
            {
                var response = new Response();
                response._result = -1;
                return response;
